...
class ItemDetailViewModel (var itemService: IItemService = ItemService()): ViewModel() {
    ...
    fun fetchAllItems() {
        viewModelScope.launch {
            items.postValue(itemService.fetchAllItems())
        }
    }
    fun fetchItems(searchTxt: String){
        viewModelScope.launch {
            val results = itemService.fetchItems(searchTxt)
            results?.let {
                items.postValue(it)
            }
        }
    ...
    fun fetchSavedItems(){
        viewModelScope.launch {
            val results = itemService.fetchSavedItems()
            results?.let {
                items.postValue(it)
            }
        }
    ...
    fun fetchItemById(itemId : Int){
        viewModelScope.launch {
            val results = itemService.fetchItembyId(itemId)
        }
    }
